...
class ClassifierSet:
	...
	def importRulePopulation(self, rules):
		self.popSet = []
		for r in rules:
			self.popSet.append(Classifier.fromGenericObject(r))

	def toGenericObject(self):
		o = {
			"popSet": [],
			"matchSet": self.matchSet,
			"correctSet": self.correctSet,
			"microPopSize": self.microPopSize,
			"aveGenerality": self.aveGenerality,
			"expRules": self.expRules,
			"attributeSpecList": self.attributeSpecList,
			"attributeAccList": self.attributeAccList,
			"avePhenotypeRange": self.avePhenotypeRange,
			"ga": self.ga
		}
		for el in self.popSet:
			o["popSet"].append(el.toGenericObject())
		return o
	...
	def deleteFromPopulation(self):
		...
		for i in range(len(voteList)):
			...
			if newSum > choicePoint: #Select classifier for deletion
				#Delete classifier----------------------------------
				self.deletionParams.append(
					{
						"deletion-index": i,
						"deleted-classifier":self.popSet[i].toGenericObject()
					}
				)
	...
	def runGA(self, exploreIter, state, phenotype):
		...
		self.setIterStamps(exploreIter) #Updates the iteration time stamp for all rules in the correct set (which the GA operates in).
		changed = False
		...
		self.ga["parents"] = [clP1.toGenericObject(), clP2.toGenericObject()]
		...
		self.ga["offsprings-init"] = [cl1.toGenericObject(), cl2.toGenericObject()]
		...
		self.ga["offspring-parameter"] = [cl1.toGenericObject(), cl2.toGenericObject()]
		...
		self.ga["offspring-mutation"] = [
			cl1.toGenericObject().copy(),
			cl2.toGenericObject().copy()
		]
